...
public class Clustering {
     ...
     */
    void initialize(){
        //calculate compress size of each cluster
        size=0;
        for(int i=0;i<clusters.size();i++){
            ...
            //clusters.get(i).compress_size+=lz.encode(); // the cost of encoding this cluster stream  
            size+=clusters.get(i).compress_size;
           // System.out.println(clusters.get(i).compress_size);
        }
        //calculate the compress size of  merging this cluster with the other clusters.
        for(int i=0;i<clusters.size();i++){
            ...
            for(int j=i+1;j<clusters.size();j++){
                ...
                //if(i==0&&j==1){
                    //System.out.println(sz+" "+clusters.get(i).compress_size+" "+clusters.get(j).compress_size);
                   // lz.printDictionary();
                //}
                clusters.get(i).merge_size.add(sz);
                /*if(clusters.get(i).alphabet.get(0).intValue()==322&&clusters.get(j).alphabet.get(0).intValue()==518||
                        clusters.get(i).alphabet.get(0).intValue()==518&&clusters.get(j).alphabet.get(0).intValue()==322){
                    for(int iii=0;iii<lz.seq.size();iii++)
                        System.out.print(lz.seq.get(iii).id+" ");
                     System.out.println();
                     System.out.println(sz+" ha ha " +clusters.get(i).uncompress_size+" "+clusters.get(j).uncompress_size);
                     lz.printDictionary();
                }*/
            }
            if(i%100==0&&i>0)
                System.out.println(i +" clusters have been already initialized");
        }
    }
    ...
    void hierarchical_clustering(){
        ...
        while(true){
            ...
            System.err.println("Current size... "+ size + " bits");
            max=0;
            for(int i=0;i<clusters.size();i++){
                for(int j=i+1;j<clusters.size();j++){
